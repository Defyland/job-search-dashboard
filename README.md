...
The deterministic Rails backfill can already be run manually from the dashboard or via `dashboard:discover`. The Runs screen can launch either a full backfill or a source-scoped backfill, and the Sources screen can do the same right after you change adapter settings. The Sources screen is now the operational place to seed adapter-specific `settings` such as `board_urls`, `company_labels`, `company_slugs`, `company_identifiers`, `search_queries`, and `max_pages`. `dashboard:seed_sources` is now non-destructive for existing records: it bootstraps missing catalog fields but preserves operator overrides for adapter, priority, enablement, scan window, host/base URL edits, and JSON `settings`. If you need to roll out a new default over an existing source, use an explicit migration or task instead of relying on deploy-time seeding. Codex ingestion remains available as a complementary API path, but there is no longer a scheduled Codex automation feeding the app.
